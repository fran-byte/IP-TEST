...
class PingPong():
    ...
    def enviar_ping(self, i=1):
        ...
        if check_multiple_ethernet_adapters():
            print("Attention, First, Save SETTINGS.")
        ...
        if response_gateway == 0:
            label_resultado.config(text=f"Successful Connection - {gateway}", fg="#F0F0F0", bg="dark green")
            self.pings_erroneos = 0  # Reiniciamos el contador de pings incorrectos
        else:
            label_resultado.config(text=f"Connection Error - {gateway}", fg="#F0F0F0", bg="dark red")
...
button_dhcp.grid(row=11, column=1, columnspan=2)
...
boton_habilitar_wifi.grid(row=8, column=1, columnspan=2, pady=10)
...
label_advertencia.grid(row=12, column=0, padx=125, pady=0)
